...
Gradients = OrderedDict([
    ...
    ('cyclic', {'ticks': [(0.0, (255, 0, 0, 255)), (0.25, (183, 0, 255, 255)), (0.5, (0, 234, 255, 255)),
                            (0.75, (174, 255, 0, 255)), (1.0, (255, 0, 0, 255))], 'mode': 'rgb'}),
    ('greyclip', {'ticks': [(0.0, (0, 0, 0, 255)), (0.5, (255, 255, 255, 255)), (1.0, (255, 0, 0, 255))], 'mode': 'rgb'}),
...
class color_gradient_builder(object):
    def __init__(self, ticks, kind):
        ...
        self.gf = None
        self.bf = None
        self.build(ticks, kind)

    def build(self, ticks, kind):
        r = []
        g = []
        b = []
        pos = []
        for t, p in ticks:
            ...
            g.append(c[1])
            b.append(c[2])
        self.rf = build_curve_ramp(r, pos, kind)
        self.gf = build_curve_ramp(g, pos, kind)
        self.bf = build_curve_ramp(b, pos, kind)

    def getQColors(self, w):
        if self.rf is None or self.gf is None or self.bf is None:
            return None
        w /= 2.0
        return [(i / w, QtGui.QColor.fromRgbF(self.rf(i / w), self.gf(i / w), self.bf(i / w))) for i in range(int(w))]

    def getColor(self, pos):
        if self.rf is None or self.gf is None or self.bf is None:
            return QtGui.QColor.fromRgbF(0, 0, 0)
    ...
    **Signals:**
    ...
    def __init__(self, *args, **kargs):
        ...
        **Arguments:**
        ...
        self.rectSize = 15
        if 'height' in kargs.keys():
            self.rectSize = kargs['height']
        self.kind = 'linear'
        ...
        self.addTick(1, QtGui.QColor(255, 255, 255), True)
    ...
    def value(self):
        colors = []
        pos = []
        for t, p in self.listTicks():
            if pos and p == pos[-1]:
                continue
            pos.append(p)
            colors.append(t.color.getRgbF())
        return [colors, pos, self.kind]
    ...
    def setValue(self, value):
        ...
        for i in range(len(pos)):
            x = pos[i]
            c = colors[i]
            self.addTick(x, QtGui.QColor.fromRgbF(*c), finish=False)
        self.updateGradient()
        self.sigGradientChangeFinished.emit(self)
    ...
    def getGradient(self):
        ...
        if self.colorMode == 'rgb':
            ...
            if len(ticks) == 0 or self._use_default:
                g.setStops([(x, QtGui.QColor(t.color)) for t, x in ticks])
            else:
                b = color_gradient_builder(ticks, self.kind)
                stops = b.getQColors(self.width())
                if stops is None:
                    return
                g.setStops(stops)
...
class Tick(QtGui.QGraphicsWidget):  ## NOTE: Making this a subclass of GraphicsObject instead results in
    ...
    def mouseDragEvent(self, ev):
        if self.movable and ev.button() == QtCore.Qt.LeftButton:
            ...
            ev_pos = self.mapToParent(ev.pos())
            newPos = self.cursorOffset + ev_pos
            newPos.setY(self.pos().y())
            self.drag_delta = ev_pos.y() - newPos.y()
            self.setPos(newPos)
            self.view().tickMoved(self, newPos)
            self.sigMoving.emit(self)
            if ev.isFinish():
                self.moving = False
                if self.drag_delta > 40:
                    self.view().removeTick(self)
                    return
                self.sigMoved.emit(self)
